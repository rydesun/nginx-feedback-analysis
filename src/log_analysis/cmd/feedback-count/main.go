...
func parseLog(fr io.Reader) LangCount {
	...
	if err != nil {
		dla.Logger.Fatal(errors.Wrap(err, "new gzip reader failed"))
	}
	defer gzr.Close()
...
func main() {
	cfg, err := dla.LoadDefaultConfig()
	if err != nil {
		dla.Logger.Fatal(errors.Wrap(err, "load config failed"))
	}

	db, err := dla.ConnectDB(cfg)
	if err != nil {
		dla.Logger.Fatal(errors.Wrap(err, "connect db failed"))
	}

	resp, err := http.Get(cfg.LogWatch.UrlBase)
	if err != nil {
		dla.Logger.Fatal(errors.Wrap(err, "get log page failed"))
	}
	if resp.StatusCode != http.StatusOK {
		dla.Logger.Fatal("response status: ", resp.Status)
	}
